...
export type UserRole = 'user' | 'support' | 'admin' | 'super_admin' | 'specialist' | 'coordinator';
...
export function SpecialistAuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadSpecialistUser = async (userId: string): Promise<SpecialistUser | null> => {
    try {
      ...
      if (userData.role === 'specialist' || userData.role === 'coordinator') {
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (data.user) {
      ...
      if (!userData || !['specialist', 'coordinator'].includes(userData.role)) {
  ...
  const isCoordinator = specialistUser?.role === 'coordinator';
