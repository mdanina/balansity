...
export default function Blog() {
  ...
  const filterPostsBySearch = (postsToFilter: typeof posts) => {
  ...
  // Применяем поиск к постам
  const filteredPosts = filterPostsBySearch(posts);

  // Ищем featured статью, если её нет - берем первую
  const hero = filteredPosts?.find(p => p.is_featured) || filteredPosts?.[0];
  const rest = filteredPosts?.filter(p => p.id !== hero?.id) ?? [];

  // Логируем данные для отладки
  useEffect(() => {
    if (posts && posts.length > 0) {
      const problematicPost = posts.find(p => p.title === 'иииооо' || p.title.includes('иии'));
      if (problematicPost) {
        console.log('[Blog] Проблемная статья найдена:', {
          id: problematicPost.id,
          title: problematicPost.title,
          cover_image_url: problematicPost.cover_image_url,
          slug: problematicPost.slug,
          allFields: problematicPost
        });
      }
    }
  }, [posts]);
            ...
            <div className="max-w-7xl mx-auto">
              <div className="blog-hero-card grid gap-8 md:grid-cols-[minmax(0,1.7fr)_minmax(0,1.3fr)] items-center">
                <div className="relative z-10 space-y-4">
                  <div className="blog-hero-pill text-xs font-medium text-black/70">
                    <span>Журнал Balansity</span>
                    <span className="h-1 w-1 rounded-full bg-white/70" />
                    <span>для тех, кто тащит на себе всё</span>
                  </div>
                  <h1 className="font-serif text-3xl font-bold leading-tight text-[var(--landing-dark-blue)] md:text-4xl lg:text-5xl">
                    Как жить, работать и растить детей,
                    <br className="hidden md:block" /> не уходя в эмоциональный выгорание
                  </h1>
                  <p className="max-w-xl text-sm text-[#222327]/80 md:text-base">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 text-xs font-medium text-[#15161a]">
                    {/* Кнопка "Все статьи" */}
                    <button
                      type="button"
                ...
                {hero && (() => {
                  ...
                  return (
                    <Link
                      to={`/blog/${hero.slug}`}
                      className="relative z-10 flex h-full rounded-3xl overflow-hidden bg-white text-left flex-col"
                    >
                      {/* Обложка */}
                      {hero.cover_image_url ? (
                        ...
                        <div className="flex-1 min-h-[200px] max-h-[300px] flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        <p className="mb-2 text-[0.7rem] font-semibold uppercase tracking-[0.18em] text-black/70">
                          Главный материал сейчас
                        </p>
                        <h2 className="mb-2 line-clamp-3 font-serif text-lg font-semibold text-[#15161a] md:text-xl">
                          {hero.title}
                        </h2>
                        {hero.subtitle && (
                          <p className="mb-3 line-clamp-3 text-xs text-[#222327]/80">
                            {hero.subtitle}
                          </p>
                        )}
                        <div className="mt-auto flex flex-wrap items-center gap-2 text-[0.7rem] text-[#15161a]/80">
                          {category && (
                            <span className="rounded-full bg-black/8 px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wide">
                              {category.label}
                            </span>
                          )}
                      ...
                      <div className="blog-card-cover">
                        {post.cover_image_url ? (
                          <img
                            ...
                            onError={(e) => {
                              console.error('[Blog] Ошибка загрузки изображения:', {
